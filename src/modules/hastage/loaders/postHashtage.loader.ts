...
import { PostHastageInput } from '../inputs/HashtagPost.input.dto'
import { Like } from 'src/modules/like/entity/like.entity '
import { Hashtag } from '../entity/hastage.entity'

@Injectable()
export class PostHashtagLoader {
  private loader: DataLoader<number, PostHastageInput>
  ...
  constructor (
    ...
    this.loader = new DataLoader<number, PostHastageInput>(
      async (keys: number[]) => {
        ...
        const postIds = [...new Set(hashtags.map(hashtag => hashtag.postId))]
  ...
  load (id: number): Promise<PostHastageInput> {
    return this.loader.load(id)
  }

  async loadMany (ids: number[]): Promise<PostHastageInput[]> {
    const results = await this.loader.loadMany(ids)
    return results.filter(
      (result): result is PostHastageInput => !(result instanceof Error),
    )
  }
}
